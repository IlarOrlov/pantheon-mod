...
- **Teams** — when "Split into teams" is on, sharing (inventory, equipment, hotbar ownership, health, hunger, all of it) is scoped per-team instead of one pool for the whole server. Manage teams with `/pantheon team create|delete|list|join|leave|assign`. With teams off, everyone is effectively on one implicit team - the original server-wide-pool behavior.
...
- **Command**: `/pantheon config` shows current settings; `/pantheon config <setting> <value>` changes one (op-only). Settings: `syncArmor`, `syncOffhand`, `enableHotbarOwnership`, `syncHealth`, `syncHunger`, `teamsEnabled` (all booleans).
...
Assignments are remembered even while `teamsEnabled` is off (they just aren't used until it's on), and are lost on server restart along with everything else Pantheon tracks in memory. If hotbar ownership is also on, a team is capped at 9 online members the same way the whole server used to be.
