...
- **Teams** — when "Split into teams" is on (the default), sharing (inventory, equipment, hotbar ownership, health, hunger, XP, all of it) is scoped per-team instead of one pool for the whole server. Manage teams with `/pantheon team create|delete|list|join|leave|assign`. With teams off, everyone is effectively on one implicit team - the original server-wide-pool behavior. Which teams exist and who's on them is saved and reloaded every time the server (re)starts.
...
**On a brand new install** (no `config/pantheon.json` yet), everything defaults to off except `teamsEnabled` (on) and the client-local low-health warning (on) — armor, offhand, health, hunger, and XP sharing all start disabled so a fresh server doesn't surprise anyone; turn on whichever ones you want.
